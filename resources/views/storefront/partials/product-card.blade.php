...
<div class="product-card card flex flex-col relative group h-full">
    ...
    <a href="{{ route('product.show', $product) }}" class="block" tabindex="-1" aria-hidden="true">
        <div class="card-img-box">
            <img class="card-img-contain" src="{{ $product->imageUrl() }}" alt="{{ $product->name }}" loading="lazy" />
        </div>
    </a>
    ...
    <div class="p-5 flex flex-col flex-grow justify-between">
        <div>
            <a href="{{ route('product.show', $product) }}">
                <h3
                    class="text-sm font-bold text-[#001F54] hover:text-[#003B7A] transition-colors leading-snug line-clamp-2 mb-3">
                    {{ $product->name }}
                </h3>
            </a>
        ...
        @if ($productHasVariants)
            ...
            <a href="{{ route('product.show', $product) }}" class="primary-btn w-full justify-center text-sm">
