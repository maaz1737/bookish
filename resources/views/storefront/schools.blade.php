...
@section('content')
    ...
    <div class="flex items-start gap-4 mb-10">
        ...
        <div
            class="w-16 h-16 rounded-2xl bg-orange-50/60 border border-orange-100 flex items-center justify-center shrink-0">
            <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg"
                class="w-11 h-11 stroke-[#e06c00] fill-none" stroke-width="2" stroke-linecap="round"
    ...
    <div id="con" class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach ($schools as $school)
            <a href="{{ route('schools.show', $school) }}"
                class="group bg-white rounded-3xl border border-slate-100 hover:-translate-y-0.5 transition-all duration-300 p-6 flex flex-col justify-between school-card">

                <div class="flex items-center gap-5">
                    <!-- School Logo Container -->
                    <div
                        class="w-20 h-20 rounded-2xl border border-slate-100 flex items-center justify-center bg-white p-3 shrink-0 shadow-sm">
                        @if ($school->logo)
                            <img src="{{ asset('storage/' . $school->logo) }}" class="max-w-full max-h-full object-contain"
                                alt="{{ $school->name }}">
                        @else
    ...
    @if ($schools->count() == 0)
        <div
            class="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-16 text-center max-w-lg mx-auto">
    ...
    <div
        class="mt-16 bg-[#f0f4fa]/60 border border-blue-100/50 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
            <!-- Headset support icon in a circular container -->
            <div
                class="w-14 h-14 rounded-full bg-white shadow-sm border border-blue-100 flex items-center justify-center text-navy-800 shrink-0">
        ...
        <!-- Contact Us Button -->
        <a href="#"
            class="shrink-0 flex items-center gap-2 bg-[#0a1a3d] hover:bg-[#1e3a8a] text-white px-6 py-3.5 rounded-xl text-sm font-semibold shadow-sm transition-colors duration-300">
