    ...
    <section class="max-w-7xl mx-auto px-4 mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            @foreach ($schools as $i => $school)
                ...
                <a href="{{ route('schools.show', $school) }}" class="{{ $bg }} rounded-xl p-5 flex flex-col justify-between">
                    <div class="flex items-start gap-4">
                        <div class="logo-box bg-white p-2 rounded-lg shadow-sm w-12 h-12 flex items-center justify-center shrink-0">
                            @if($school->logo ?? false)
                                <img src="{{ asset('storage/' . $school->logo) }}" alt="{{ $school->name }}" class="max-w-full max-h-full object-contain">
                            @else
                                <i class="fa-solid fa-school text-xl text-navy-700"></i>
                            @endif
                        </div>
                        <div class="flex-1">
                            <h3 class="font-bold text-navy-900">{{ $school->name }}</h3>
                            <ul class="text-sm text-slate-600 mt-2 space-y-1">
                                <li><i class="fa-solid fa-book text-navy-600 mr-1 text-xs"></i> Books</li>
                                <li><i class="fa-solid fa-shirt text-navy-600 mr-1 text-xs"></i> Uniforms</li>
                                <li><i class="fa-solid fa-bag-shopping text-navy-600 mr-1 text-xs"></i> Accessories</li>
                            </ul>
                        </div>
                    </div>
                    <span class="mt-5 block text-center bg-navy-800 hover:bg-navy-900 transition text-white py-2.5 rounded-md text-sm font-semibold">View Classes <i class="fa-solid fa-arrow-right ml-1"></i></span>
                </a>
            @endforeach
        </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-4 mb-12">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            @foreach ($categories as $category)
                <div class="bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-md transition flex flex-col justify-between">
                    <div>
                        <div class="card-img-box bg-slate-50 aspect-video w-full overflow-hidden flex items-center justify-center">
                            <img class="w-full h-full object-cover" src="{{ app()->environment('production')
                                ? url('storage/app/public/' . $category->image)
                                : asset('storage/' . $category->image) }}" alt="{{ $category->name }}">
                        </div>
                        <div class="p-4">
                            <div class="flex items-start gap-2">
                                <span class="w-8 h-8 rounded-full bg-navy-50 flex items-center justify-center text-navy-700 shrink-0"><i class="fa-solid fa-book text-xs"></i></span>
                                <div>
                                    <h3 class="font-bold text-navy-900 text-sm line-clamp-1">{{ $category->name }}</h3>
                                    <p class="text-xs text-slate-500 mt-0.5">{{ \Illuminate\Support\Str::limit($category->description, 30, '...') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="p-4 pt-0 text-center">
                        <a href="{{ route('category.show', $category->slug) }}" class="inline-flex items-center text-xs font-semibold text-navy-700 hover:text-navy-900">Explore Now <i class="fa-solid fa-arrow-right ml-1"></i></a>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <section class="max-w-7xl mx-auto px-4 mb-12">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
                @foreach ($bestSellers as $product)
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-md transition relative flex flex-col justify-between">
                        ...
                        <div>
                            <div class="card-img-box p-5 bg-slate-50 aspect-square flex items-center justify-center">
                                <img class="max-w-full max-h-full object-contain" src="{{ asset( $product->image) }}" alt="{{ $product->name }}">
                            </div>
                            <div class="p-4">
                                <h3 class="font-semibold text-navy-900 text-sm line-clamp-1">{{ $product->name }}</h3>
                                ...
                                <p class="mt-2 font-bold text-navy-900 text-sm">PKR {{ number_format($product->price) }}</p>
                            </div>
                        </div>
                        <div class="p-4 pt-0">
                            <button class="w-full border border-navy-200 text-navy-800 py-2 rounded-md text-xs font-semibold hover:bg-navy-50 transition">
