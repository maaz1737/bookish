    ...
    <form method="POST" action="{{ route('admin.bundles.store') }}" class="bg-white p-6 rounded-lg shadow space-y-4">
        ...
        <div class="grid sm:grid-cols-3 gap-4">
            <label class="block"><span class="text-sm font-medium">School</span>
                <select name="school_id" id="school" required class="w-full border rounded px-3 py-2 mt-1">
                    ...
                    @foreach ($schools as $s)
                        <option value="{{ $s->id }}"
                            {{ old('school_id', $bundle->school_id) == $s->id ? 'selected' : '' }}>
                            {{ $s->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <label class="block"><span class="text-sm font-medium">Class</span>
                <select name="class_id" id="class" required class="w-full border rounded px-3 py-2 mt-1">
                    @foreach ($schools->firstWhere('id', $bundle->school_id)?->classes ?? [] as $class)
                        <option value="{{ $class->id }}"
                            {{ old('class_id', $bundle->class_id) == $class->id ? 'selected' : '' }}>
                            {{ $class->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <label class="block"><span class="text-sm font-medium">Discount %</span>
                <input name="discount" type="number" step="0.01" value="{{ old('discount', $bundle->discount) }}"
                ...
                @foreach ($products as $i => $p)
                    ...
                    <label class="flex items-center justify-between text-sm">
                        ...
                        <input type="number" name="items[{{ $i }}][quantity]"
                            value="{{ $bundleItem?->quantity ?? 1 }}" min="1" class="w-16 border rounded px-2 py-1">
                    </label>
                @endforeach
            </div>
        </div>
