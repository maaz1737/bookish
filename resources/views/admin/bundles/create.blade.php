...
@section('content')
    <h1 class="text-2xl font-bold mb-2">Build a Bundle</h1>
    <p class="text-sm text-gray-500 mb-6">Create a combination of any 2 or more products with a custom name and bundle discount.</p>
    
    <form method="POST" action="{{ route('admin.bundles.store') }}" class="bg-white p-6 rounded-lg shadow space-y-6">
        @csrf
        
        <div class="grid sm:grid-cols-3 gap-6">
            <label class="block sm:col-span-3">
                <span class="text-sm font-bold text-gray-700">Bundle Name <span class="text-red-500">*</span></span>
                <input name="name" type="text" placeholder="e.g. Smart Saver Bundle" required value="{{ old('name') }}"
    ...
    <script>
        ...
        // Auto-run if school_id was old selected
        if (schoolSel.value) {
            updateClasses("{{ old('class_id') }}");
        }
