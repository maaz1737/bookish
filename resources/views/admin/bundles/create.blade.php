...
<form method="POST" action="{{ route('admin.bundles.store') }}" class="bg-white p-6 rounded-lg shadow space-y-4">
    ...
    <div class="grid sm:grid-cols-3 gap-4">
        ...
        <label class="block"><span class="text-sm font-medium">Class</span>
            <select name="class_id" id="class" required class="w-full border rounded px-3 py-2 mt-1"></select></label>
        <label class="block"><span class="text-sm font-medium">Discount %</span>
            <input name="discount" type="number" step="0.01" value="10" required class="w-full border rounded px-3 py-2 mt-1"></label>
    </div>
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto border rounded p-3">
            @foreach ($products as $i => $p)
                <label class="flex items-center justify-between text-sm">
                    <span><input type="checkbox" name="items[{{ $i }}][product_id]" value="{{ $p->id }}"> {{ $p->name }}
                        <span class="text-gray-400">({{ number_format($p->effectivePrice()) }} PKR)</span></span>
                    <input type="number" name="items[{{ $i }}][quantity]" value="1" min="1" class="w-16 border rounded px-2 py-1">
                </label>
            @endforeach
        </div>
    </div>
...
const schoolSel = document.getElementById('school'), classSel = document.getElementById('class');
schoolSel.addEventListener('change', () => {
    classSel.innerHTML = '';
    (classesBySchool[schoolSel.value] || []).forEach(c => {
        const o = document.createElement('option'); o.value = c.id; o.textContent = c.name; classSel.appendChild(o);
    });
});
</script>
@endsection
